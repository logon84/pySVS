...
#EDIT THIS VALUE#####################
SVS_MAC_ADDRESS = "08:eb:ed:62:b3:f5"
#####################################
...
async def gatt_thread(address, char_uuid):
    async with BleakClient(address) as client:
        ...
        await device.write_gatt_char(CHAR12, svs("ASK", "VOLUME"))
        await asyncio.sleep(0.1)
        await device.write_gatt_char(CHAR12, svs("ASK", "PHASE"))
        await asyncio.sleep(0,1)
        await device.write_gatt_char(CHAR12, svs("ASK", "LOW_PASS_FILTER_ALL_SETTINGS"))
        await asyncio.sleep(0,1)
        await device.write_gatt_char(CHAR12, svs("ASK", "ROOM_GAIN_ALL_SETTINGS"))
        await asyncio.sleep(0,1)

        while RUN_THREAD:
        #don't let this method die in order to RX continuosly
             if len(update_SEND.SEND) > 0: 
                 await device.write_gatt_char(CHAR12, update_SEND.SEND)
                 update_SEND.SEND = ""
...
def update_vol(self):
    update_SEND.SEND = svs("SET","VOLUME",volume2hex(vol_slider.get()))

def update_phase(self):
    update_SEND.SEND = svs("SET","PHASE",phase2hex(phase_slider.get()))

def lfe_opt_changed():
    refresh_conditional_widgets()
    update_SEND.SEND = svs("SET","LFE",lfe_state2hex(lfe_var.get()))
...
def update_lpfilter_freq(self):
    ...
    #as this callback is called when the click is released, be sure only to send svs set only if lfe = off
        update_SEND.SEND = svs("SET","LOW_PASS_FILTER_FREQ",lp_freq2hex(lpfilter_slider.get()))

def update_lpfilter_slope(self):
    update_SEND.SEND = svs("SET","LOW_PASS_FILTER_SLOPE",lpfilter_slope2hex(lpfilter_slope_combo.current()))

def room_gain_opt_changed():
    refresh_conditional_widgets()
    update_SEND.SEND = svs("SET","ROOM_GAIN_ENABLE", room_gain_state2hex(room_gain_var.get()))
...
def update_room_gain_freq(self):
    ...
    #as this callback is called when the click is released, be sure only to send svs set only if room_gain = on
        update_SEND.SEND = svs("SET","ROOM_GAIN_FREQ", room_gain_freq2hex(room_gain_slider.get()))

async def update_room_gain_slope(self):
    update_SEND.SEND = svs("SET","ROOM_GAIN_SLOPE", room_gain_slope2hex(room_gain_slope_combo.current()))
